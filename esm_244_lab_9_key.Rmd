...
# Note: may need to update dependencies (I usually choose yes to all...) # yep, I did this
...
les_mis <- graph_from_data_frame(lm_df, directed = FALSE) # directed = FALSE, no directionality in the graph theory
beep(sound = 8) # AMAZING # this will let you know when your long code has run!
praise()

```

Find some quantitative metrics of graphs (graph theory graphs, not charts):
```{r}

# These are functions built into the graph theory packages to help learn about the data

#Graph diameter:
diameter(les_mis) # Smallest maximum distance (links) between any two characters in Les Mis, the farthest separation is 5

# Farthest vertices
farthest_vertices(les_mis) # between Napoleon and Jondrette
...
plot(les_mis) # looks like a nightmare
...
ggraph(les_mis, layout = 'kk') +
  geom_edge_link() +  # lines between things
  geom_node_text(aes(label = name), size = 2, color = "white") +
  theme_dark()

# Or an arc graph: # THIS IS SO FUN
ggraph(les_mis, layout = "linear") + 
  geom_edge_arc(alpha = 0.8) + 
  geom_node_text(aes(label = name), angle = 90, size = 2, hjust = 1) +
  theme_graph() # takes out the unnecessary parts of a network graph

# Try layout = "circle", "tree" # coerced into circular format
...
ggraph(les_mis, layout = "linear", circular = TRUE) + # layout = "linear" mean plot like a line, then make it circular
  geom_edge_arc(alpha = 0.5) +
  geom_node_point(aes(colour = name), show.legend = FALSE) +
  geom_node_text(aes(label = name), size = 3, hjust = "outward") + # hjust outward is very helpful, we could adjust outward even more
  theme_void()
...
Other packages to look into for Sankeys: alluvial, ggalluvial, NetworkD3, ggforce
...
# ggalluvial package works within ggplot

ggplot(sdf, aes(y = weight, axis1 = before, axis2 = after)) + # there can be more than two axes
  geom_alluvium(aes(fill = before, color = before), show.legend = FALSE, width = 1/5) +
  geom_stratum(width = 1/5, color = "gray") + # bins
  geom_text(stat = "stratum", label.strata = TRUE) + # bins with labels so we know where things are flowing
...
# Make a little tibble - this is useful for making a little dataframe in order to practice stuff!

my_tibble <- tribble(
  ~hanna, ~made, ~this, ~awesomething,
  1, "yes", 0, 10000,
  2, "no", 4, 50000,
  3, "no", 10, 40000,
  4, "maybe", 9, 15000
...
This is important because if you are getting help from an outside source, they don't have your data or your file paths. This can help you create a small, reproducible example of a line that doesn't work. Tribble() can help you make a little df that they can work with.

####3b. But now, datapasta! 

Can I copy and paste data from outside sources and have R store it in a dataframe? Sometimes!
...
# there is no way to link this to an updated data source, so be careful # always keep a record with metadata and store this as a csv
...
write_csv(hanna_datapasta, "hanna_storing_things.csv") # this can be pushed back to github, and make a txt file with metadata
```
...
R is lacking in creating beautiful tables
...
weather_data %>% 
  gt() %>% 
  tab_header(
    title = "Pretty cool table",
    subtitle = "Made a subtitle"
  ) %>% 
  fmt_number(
    columns = vars(Min),
    decimals = 2 # omg
...
Or make it way better (Allison's code from the key: 
...
Make sure you read in and keep a history of dates, etc. so that you can update.
...
###3e. Reading in Excel files (and skipping top rows metadata) OH MY GOSH YES
...
# this means that all data wrangling is happening in R which is more reproducible
...
crops <- pest %>% 
  ...
  separate(grower_city, 
           ...
           sep = ",") %>% # from 'str' package # break up column into two columns (any state information)
  separate(collection_site_city, 
           c("market_city","market_state"), 
           sep = ",") %>% 
  group_by(organic_commodity, grow_city, market_city) %>% # very different df, but more useful
  tally()
